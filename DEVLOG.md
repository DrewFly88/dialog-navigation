...
### §32.5h.1 官�远距离跳转为何更快（2026-07-19 补调研）

实测从官�prev button React fiber 拿到 `onClick` 闭包源码：

```javascript
onClick: "fn: function(){return le&&ae(le.id)}"
```

官�prev/next 的跳转实现是 `ae(le.id)`——`le` 是当前要跳的目标 message 对象，`le.id` 是消息位置 key，`ae` 是 `onEnsureMessageVisible` 的内层引用。

### 根因分析

| 维度 | 官�prev/next | 本插件 scroll loop |
|:--|:--|:--|
...
**官�更快的根因**：BubbleList 内部维护 `items[]` 数组含全部消息位置 key——跳转时直接调 `scrollIntoView` 触 SDK 内部虚拟列表渲染（目标消息位置已知，SDK 内部高效定位），无需逐页 loadMore。本插件的 scroll loop 是「从外部 DOM 盂 SDK 渲染」——逐页 loadMore 触发 SDK re-fetch + 等渲染，慢是因为绕了 SDK 内部机制。
