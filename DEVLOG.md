...
## §32.5e 待办：视口滚动时同气泡多结论不跟随高亮（暂不修复，2026-07-12）

### 问题描述

视口滚动时，同一气泡中多条结论条目不会跟随视口改变而改变高亮——只滚到气泡时高亮一条，之后在气泡内滚动不会切换到视口实际可见的另一条结论。对比工具分组（tool）的视口跟踪：同卡片内多工具调用会按 viewportCenter 实时切换高亮条目。结论分组缺这套机制。

### 根因

`useViewportTracker` 只为 tool 分组实现了「同卡片内多元素的高亮切换」：

| 机制 | tool 分组 | conclusion 分组 |
|:--|:--|:--|
| IntersectionObserver 监元素 | ✅ 监 `details[class*="toolCallCompact"]` | ❌ 未监 `<strong>`/`<li>` |
| visibleRefs 存 childIdx | ✅ `visibleToolCallsRef` 存 `{agentPos, childIdx}` | ❌ 无 visibleConclusionRef |
| calculate 找最近 | ✅ 按 `viewportCenter` 找最近 tool 的 `childIdx` | ❌ 只算 `activeBubbleIndex`，无 `activeConclusionIndex` |
| 返回值 | ✅ `[activeBubbleIndex, activeToolIndex]` | ❌ 无 `activeConclusionIndex` |
| BarStrip activeItemId 分发 | ✅ tool 组用 `bubbleIndex AND childIndex` 精准匹配 | ❌ conclusion 组只取 `largest bubbleIndex ≤ activeBubbleIndex` |

**根因一句话**：`useViewportTracker` 和 `BarStrip` 都没有为 conclusion 分组做「同卡片内多元素的高亮切换」实现，只做了气泡级（bubbleIndex）跟踪，没做元素级（childIndex）跟踪。

### 拟定解决方案（仿 tool 分组的 conclusion 视口跟踪，暂不实施）

改动清单（4 文件）：

| 文件 | 改动 |
|:--|:--|
| `src/index.tsx` | 把 `activeGroup` state 从 BarStrip 提到此处；`useViewportTracker` 调用加传 `activeGroup`；接收 `activeConclusionIndex`；传 `activeGroup`/`setActiveGroup`/`activeConclusionIndex` 给 BarStrip |
| `src/hooks/useViewportTracker.ts` | 签名加 `activeGroup?: string` 参数；加 `activeConclusionIndex` state；`calculate` 内若 `activeGroup === "conclusion"`，算当前 active card 内最近 conclusion 元素的 childIdx（复用 §32 三优先级分抓 + C2 筛 + userQuote 跳）；返回 `[activeBubbleIndex, activeToolIndex, activeConclusionIndex]` |
| `src/BarStrip.tsx` | props 加 `activeGroup`/`setActiveGroup`/`activeConclusionIndex`；删内部 `activeGroup` state；`activeItemId` 分发：conclusion 组用 `bubbleIndex AND childIndex` 精准匹配（仿 tool 组） |
| `src/parser/conclusionParser.ts` | 无改动（parser childIdx 算法不变） |

关键算法（`useViewportTracker.calculate` 内，避免监大量 `<strong>`/`<li>`，只在 scroll 触发时算）：

```typescript
if (activeGroup === "conclusion" && closestAgentIdx >= 0) {
  const cardEl = findCardElByParserIdx(closestAgentIdx);
  if (cardEl) {
    // 复用 §32 三优先级分抓 + C2 筛 + userQuote 跳
    const boldEls = Array.from(cardEl.querySelectorAll('strong'));
    const liEls = Array.from(cardEl.querySelectorAll('li'));
    const numberedEls = liEls.filter(el => numberedLi.test(...));
    const plainLiEls = liEls.filter(el => !numberedLi.test(...));
    let matchIdx = 0, closestChildIdx = -1, closestDist = Infinity;
    for (const el of [...boldEls, ...numberedEls, ...plainLiEls]) {
      ...
      if (dist < closestDist) { closestDist = dist; closestChildIdx = matchIdx; }
      matchIdx++;
    }
    setActiveConclusionIndex(prev => closestChildIdx !== prev ? closestChildIdx : prev);
  }
}
```

风险评估：性能可控（只在 scroll calculate 时算，不监大量元素）；序号对齐复用 §32 精准段同款算法；BarStrip 提 state 是 React 惯例无副作用。预期效果：视口滚动时同气泡多结论条目按 viewportCenter 找最近 conclusion 元素高亮，仿 tool 分组行为。

**注**：本方案暂不实施——§32.5d 发现 §32 以来的结论精准定位改动像在不断打补丁，verdict 正则字符类 bug、tagPattern 误筛、stall 放宽……越修越觉得整体方案需重做。先全面复盘 §32+ 所有改动对工具分组的影响，统一处理后再决定问题 2 是否按此方案实施或换思路。

---

## §32.5e 工具分组精准定位修复：textContent 去重 + 最新卡 selector 兜底（2026-07-12）

### 背景

全面复盘 §32+ 所有改动对工具分组的影响，逐一实测 20 条工具条目（覆盖 bi=1~77 远近卡），发现两个 bug：

### 问题 A：textContent 去重 vs call_id 去重不匹配

**症状**：tool-9（ci=6）往后条目 `precision 匹配成功` 但 `navigated to` 命中的是错误工具——精准段匹配了 `matchIdx === childIndex`，但 `matchIdx` 基于 textContent 去重，与 parser 的 `call_id` 去重不同源。

**根因**：parser `toolCallParser.ts:77-81` 按 `call_id` 去重（去除 stream delta 重复），但 DOM 精准段 `index.tsx:291-303` 按 `textContent` 去重（`seen` Set 去重标签文本）。当同一工具函数被多次调用且参数相同时，parser 保留每次调用（不同 `call_id`），但 DOM 跳过重复文本——`matchIdx` 比 `childIndex` 小，导致高 ci 条目匹配到错误元素。

**修复**：删 `textContent` 去重，改用索引匹配——DOM 候选序号与 parser `childIndex` 1:1 对齐。

### 问题 B：`[class*="toolCallLabel"]` 在新卡中匹配不到元素

**症状**：bi=77（最新卡）所有条目 `precLog: ""`，精准段完全跳过，`precisionTarget` 未设置，退化到卡级滚动。

**根因**：SDK 对最新卡渲染可能使用不同 class 命名，`selector = '[class*="toolCallLabel"]'` 匹配不到任何元素，`candidates` 为空。

**修复**：selector 加兜底——`[class*="toolCallLabel"]` 无匹配时，改用 `[class*="toolCallCompactSummary"], [class*="toolCallCompact"] summary`；`closest('[class*="toolCallCompact"]')` 改为 `closest('details')` 更稳健。

### 两问题与 §32+ 改动的关系

| 问题 | 根因 | 引入时间 |
|:--|:--|:--|
| A: textContent 去重 | §32 原始精准段设计缺陷 | §32 引入 |
| B: 最新卡 selector 不匹配 | SDK 渲染差异，预存在 | 预存在 |

两问题均与 §32.5c 的 observer 无关——通过回退 observer 测试验证（移除 observer 后问题仍在）。
...
### §32.5h.1 官方远距离跳转为何更快（2026-07-19 补调研）

实测从官方prev button React fiber 拿到 `onClick` 闭包源码：

```javascript
onClick: "fn: function(){return le&&ae(le.id)}"
```

官方prev/next 的跳转实现是 `ae(le.id)`——`le` 是当前要跳的目标 message 对象，`le.id` 是消息位置 key，`ae` 是 `onEnsureMessageVisible` 的内层引用。

### 根因分析

| 维度 | 官方prev/next | 本插件 scroll loop |
|:--|:--|:--|
| 数据源 | BubbleList 内部 `items[]` 数组（已含全部消息位置 key） | API 重拉 `messages[]` + parser |
| 跳转目标 | user 消息气泡（BubbleList 内部索引直查） | agent 卡内具体元素（需 `querySelectorAll` 精准匹配） |
| 加载机制 | BubbleList 内部虚拟列表 + 渲染缓存——目标消息已在 `items` 数组中，`scrollIntoView` 触发 SDK 内部渲染 | scroll loop 逐页 `loadMore` 触发 SDK re-fetch + 等渲染 |
| 耗时 | 近瞬时（`ae(id)` 同步滚 + SDK 内部缓存命中） | ~3-5s（scroll loop 多轮 + 1500ms 稳定 + rAF×2） |

**官方更快的根因**：BubbleList 内部维护 `items[]` 数组含全部消息位置 key——跳转时直接调 `scrollIntoView` 触 SDK 内部虚拟列表渲染（目标消息位置已知，SDK 内部高效定位），无需逐页 loadMore。本插件的 scroll loop 是「从外部 DOM 盂 SDK 渲染」——逐页 loadMore 触发 SDK re-fetch + 等渲染，慢是因为绕了 SDK 内部机制。

### 借鉴方向（暂不实施）

| 可借鉴 | 实现方式 | 改动量 |
|:--|:--|:--|
| **用 BubbleList 内部 `items[]` 数组** | 从 React fiber 拿 `items` 数组含全部消息位置 key，跳转时直接 `scrollIntoView` 触 SDK 内部渲染 | 中——需扒 fiber 拿 `items` + 算 bi→items 索引映射 |
| **调 `onEnsureMessageVisible` 闭包** | 从 nav fiber 拿 `ae`（`onEnsureMessageVisible`）闭包直接调，复用 SDK 内部高效跳转 | 低——但需确认 `ae(le.id)` 的 `le.id` 怎么拿（bi→id 映射） |
| **监 BubbleList 内部 state 变化** | 不再 API 重拉，从 BubbleList fiber 拿 `items` state 监变——天然实时 | 中——React fiber 监听 hack |

最有价值的借鉴是「从 BubbleList fiber 拿 `onEnsureMessageVisible` 闭包直接调」——但需先实测确认 `ae(le.id)` 的 `le.id` 怎么从本插件的 `bubbleIndex` 映射过去。

---

## §32.5i QwenPaw v2.0 大版本更新迁移修改计划（2026-07-20，未执行）

### 背景

QwenPaw v2.0 大版本更新修改了部分插件接口和注册机制。结合官方迁移指南 `D:\QwenPaw-source\website\public\docs\plugins-migration.en.md` + 新版插件文档 `D:\QwenPaw-source\website\public\docs\plugins.en.md` + 本项目当前清单 `plugin.json` 对照，给出迁移修改意见。本轮只落盘计划，不改代码。

### 一、清单迁移（plugin.json）—— 必须改
...
```json
{
  ...
  "version": "0.1.0",
  "type": "frontend",
  "description": "对话内容结构化索引与导航",
  "author": "DrewFly88",
  "entry": { "frontend": "dist/index.js" }
}
```

**问题**：完全缺版本兼容声明。新版加载器在 `qwenpaw_version` 和 `min_version` 都缺省时将插件视为「兼容但未约束」——不拦载但日志告警，且插件市场会过滤掉无约束条目。

**迁移方案**：

| 改动 | 值 | 理由 |
|:--|:--|:--|
| 加 `qwenpaw_version.min` | `"2.0.0"` | 本插件用 `chat.response.append`（v2.0 新增 API），低版无此扩展点 |
| 加 `qwenpaw_version.max` | `"2.1.0"` | 显式上限防未来 v2.1 破坏性改动误判兼容 |
| 升 `version` | `"0.2.0"` | 引入实时更新（§32.5g）+ id 穈定化 + 条数 gate，属 minor 级功能增 |

保留 `type: "frontend"` + `entry.frontend`——新版仍支持，无需改。

### 二、后端入口——不适用

本插件纯前端（`type: "frontend"`，无 `entry.backend`），迁移指南的「backend entry exports `plugin` instance」「`register_prompt_section` 改参数序」「`register_skill_provider` 默认值写入策略」三段均不适用。

### 三、前端 API 兼容性——需验证

本插件用了以下 `window.QwenPaw.*` API，对照新版文档：

| API | 本插件用途 | 新版状态 | 需改? |
|:--|:--|:--|:--|
| `window.QwenPaw.host` | 拿 React/antd 共享运行时 | ✅ 兼容保留 | 否 |
| `window.QwenPaw.route.wrap` | 包装 `core.chat` 路由注入 DialogIndexWrapper | ✅ 兼容保留（迁移指南列出 `route` Compatible） | 否 |
| `window.QwenPaw.chat.response.append` | §32.5g 实时更新触发 debouncedRefresh | ✅ 新版保留 + 文档明确示例 `({ data, isLast }) => ...` | 否 |

**无破坏性改动**——本插件用的三个前端 API 在新版全兼容保留。

### 四、运行时差异风险点（迁移指南明示需验证）

迁移指南 FAQ 明说：「compatible public interface 只意味着方法名和参数还能调，若插件内部依赖 agent state、workspace 内部、runtime helpers、tool config 结构、**前端页面结构**，这些运行时对象可能已变——需完整功能验证」。

本插件重度依赖前端页面结构（DOM 监听 + React fiber 遍历），以下点需实测验证：

| 验证点 | 风险 | 验证方式 |
|:--|:--|:--|
| `route.wrap("core.chat", ...)` 包装后 Inner 是否仍正常挂载 | 中——新版 Chat 路由结构可能变 | 启动后看 console `[dialog-index] Wrapper RENDERED` 日志 + BarStrip 渲染 |
| `.qwenpaw-bubble-list` class 是否仍存在 | 中——新版可能改 class 命名 | 启动后 `document.querySelector('.qwenpaw-bubble-list')` 非空 |
| `.qwenpaw-bubble-start/end` class 是否仍存在 | 中——同上 | `querySelectorAll('.qwenpaw-bubble-start').length` > 0 |
| `MutationObserver` 监 bubbleList 是否仍触发 | 中——新版若改虚拟列表机制则 observer 失效 | 实测滚动触 debouncedRefresh |
| React fiber `__reactFiber`/`__reactProps` key 是否仍可遍历 | 中——新版 React 版本可能变 | 启动后 fiber 遍历拿 `onNavigate`/`indexData` 非空 |
| `chat.response.append` 回调是否真触发 | 低——API 兼容 + §32.5g 已实测 | 发新消息看条目列表实时更新 |
| API `/chats/{id}` messages[] 结构是否变 | 中——parser 依赖 `msg.type`/`content[].type`/`role` 字段 | 启动后条目列表正常解析 |
| `toolCallLabel`/`toolCallCompact` class 是否仍存在 | 中——v2.0 content-visibility 陷阱 | 跳转 tool 条目看精准段命中 |
| `details`/`summary`/`strong`/`li` 元素结构是否仍一致 | 低——HTML 标准不变 | 跳转 conclusion/code 条目看精准段命中 |

### 五、执行步骤（待执行）

1. 改 `plugin.json` 加 `qwenpaw_version` + 升 `version`
2. 启动 QwenPaw v2.0 + 部署本插件
3. 按「验证点」表逐项实测验证
4. 若 DOM class 或 React fiber key 变了，相应改 `useMessageMap`/`index.tsx` 的 selector
5. 实测通过后提交 + 更新 DEVLOG 记 §32.5i 迁移记录

### 六、风险评估

| 风险 | 等级 | 缓解 |
|:--|:--|:--|
| DOM class 命名变 | 中 | 启动后实测拿真实 class，动态适配 selector |
| React fiber 遍历失效 | 中 | 若 `__reactFiber` key 变，换用 `window.__reactInternal$` 或 React DevTools API |
| API messages[] 字段变 | 中 | 启动后看条目列表是否正常解析，若变则改 parser 字段访问 |
| content-visibility 陷阱 | 低 | §32.5c+2 已加 rAF×2 兜底，但 v2.0 可能更激进——实测视口跟踪精度 |

### 七、兼容性双向支持（可选）

若需同一份代码同时支持 legacy v1.x + new v2.0，清单可保留双字段：
...
legacy 版忽略未知 `qwenpaw_version` 字段，新版优先读 `qwenpaw_version` 仅在缺省时回退 `min_version`/`max_version`。但本插件已用 v2.0 新增 `chat.response.append`，legacy 版无此 API 会降级失效——故建议只支持 v2.0+，不保留 `min_version`。

---

## §32.5j v2.0 更新日志对本插件改进机会调研报告（2026-07-20，未执行）

### 背景

QwenPaw v2.0 大版本更新日志含 Runtime 2.0、Scroll Context、消息锚点、SidebarSessionList 虚拟化、请求超时、AgentScope middleware hook、Tauri 桌面端等数十项改动。结合 `qwenpaw-codebase` skill 的 reference（`frontend-architecture.md` + `agent-system.md` scroll context 段）+ 前端源码定位（`console/src/pages/Chat/index.tsx:2695` + `OptionsPanel/defaultConfig.ts:13`）+ 本插件现状痛点，对照分析改进机会。本轮只落盘调研报告不改代码。

### 一、Scroll Context（history.db + FTS5 + EvictionIndex）——「远端卡片不渲染」根因的解法

**更新日志原文**：Scroll Context：SQLite 支撑的工作历史（history.db），FTS5 全文搜索、eviction index、逐轮持久化，recall_history_python REPL 按需回溯旧轮次，默认 30 天保留期，启动时自动导入已有 session (#5321)

**本插件现状痛点**：§15/§18 早有定论——SDK v1.1.12 的 `historyMessages` 分页中，page=10 已到上限，渲染引擎仅渲染初始 ~7 轮卡片，后续 history 卡片 page 计数增加但不被渲染到 DOM——fiber dispatch 无法触发 SDK 实际加载。本插件用 scroll loop 逐页 loadMore 触发 SDK re-fetch + 等渲染，远距离跳转耗时 ~3-5s。

**v2.0 改进机会**：Scroll Context 的 `history.db` SQLite 持久化 + `recall_history` 工具支持「op=expand lo,hi」「op=search query,k」「op=recall_tool tool_call_id」——后端已有完整消息历史 + FTS5 全文搜索。但这是 Agent 侧工具（模型可调），不是前端 API——本插件是前端插件，不能直接调 `recall_history`。然而 `history.db` 是 SQLite 文件，理论上前端可通过 HTTP API 访问（需确认 v2.0 是否暴露 `/api/history` 或类似端点）。

**改进方向**（需验证）：
1. 查 v2.0 是否新增前端 HTTP API 访问 `history.db`——若有，本插件可绕过 SDK 分页渲染限制，直接从 history.db 拿全部消息 + FTS5 搜索，parser 不再依赖 `/chats/{id}` 的 `messages[]` 快照
2. 若无前端 API，可考虑注册一个轻量后端工具（本插件改 `type: "general"` 加 `entry.backend`）暴露 history.db 查询给前端

### 二、消息锚点（#5515）——已调研，借鉴方向明确

**更新日志原文**：消息锚点：用户轮次跳转导航器 (#5515)

**已调研结论**（DEVLOG §32.5h/§32.5h.1）：官方用 antd BubbleList 内置 `userMessageAnchors`（`variant: "navigator"`），3 按钮 nav（prev/list/next）；`onClick: "fn: function(){return le&&ae(le.id)}"`——`ae` 是 `onEnsureMessageVisible` 内层引用，`le.id` 是消息位置 key；官方更快的根因是 BubbleList 内部 `items[]` 数组已知全部消息位置 + SDK 内部高效 `scrollIntoView`，本插件绕了 SDK 内部机制从外部 DOM 盂渲染。

**改进方向**（已记 DEVLOG）：
1. 从 BubbleList fiber 拿 `onEnsureMessageVisible` 闭包直接调——复用 SDK 内部高效跳转
2. 从 BubbleList fiber 拿 `items[]` 数组——已知全部消息位置 key，无需 scroll loop 逐页加载
3. 加 prev/next 导航按钮——低改动高收益

### 三、SidebarSessionList 虚拟化（react-window）——本插件 BarStrip 可借鉴

**更新日志原文**：性能：SidebarSessionList 虚拟化（react-window）；轮询去重；日期格式化结果缓存 (#5643)

**本插件现状痛点**：BarStrip 渲染全部条目（234 条工具 + 若干 conclusion/code/topic），长对话时条目数上百，渲染 + 视口跟踪开销大；视口跟踪用 IntersectionObserver 监所有条目元素——条目多时性能开销大。

**改进方向**：
1. BarStrip 改用 `react-window` 虚拟化——只渲染视口内的条目，长对话性能从 O(全量) 降到 O(可见)
2. 视口跟踪从「监所有条目」改为「监虚拟列表的可见范围变化」——复用 react-window 的 `onItemsRendered` 回调

### 四、请求超时 + AbortSignal 传播——本插件 fetchAndParse 可加超时

**更新日志原文**：请求超时：可配置 HTTP timeout + AbortSignal 传播到后端调用 (#5764)

**本插件现状痛点**：`fetchAndParse` 的 `fetch` 无超时——网络慢或后端 hang 时插件 UI 卡死无反馈。

**改进方向**：`fetchAndParse` 加 `AbortController` + 超时（如 10s）——超时后显示「加载失败」提示而非静默卡死。

### 五、SessionItem 组件统一——本插件 session 切换可借鉴

**更新日志原文**：SessionItem 组件：所有 session 渲染器合并为一个 SessionItem 组件 (#5754)

**与本插件关系**：低——本插件不渲染 session 列表，只监 session 切换触发 `fetchAndParse`。但说明 v2.0 的 session 数据结构可能变——需实测验证 `useDialogIndex` 的 session 解析逻辑是否仍兼容。

### 六、工具错误透传——本插件 tool parser 可借鉴

**更新日志原文**：工具错误透传：工具执行的错误状态透传至前端 (#5912)

**本插件现状痛点**：`toolCallParser` 只解析成功的 tool call，忽略错误状态——用户看不到工具失败的高亮条目。

**改进方向**：`toolCallParser` 加解析错误状态的 tool call——BarStrip 用红色标记失败工具，点击跳转看错误信息。

### 七、多段流式回复拆分为独立消息框——本插件 parser 需验证

**更新日志原文**：频道：多段流式回复拆分为独立消息框 (#5487)

**与本插件关系**：中——若 v2.0 的 `messages[]` 结构因此变（一段回复拆成多条 message），本插件的 `bubbleIndex` 算法（`idx = tc - parserIdx`）可能错位。需实测验证。

### 八、移除消息气泡的 white-space: pre-wrap + assistant 消息 Markdown 换行保留

**更新日志原文**：移除消息气泡的 white-space: pre-wrap（导致双重换行）(#5596)；assistant 消息中的 Markdown 换行保留 (#5538)

**与本插件关系**：低——本插件的 `conclusionParser` 用 `BOLD_RE`/`LIST_ITEM_RE` 抓 markdown 加粗和列表项，`white-space` 和换行保留改动不影响 parser 正则。但若 markdown 渲染结构变了，精准段的 `querySelectorAll('strong, li')` 候选可能变——需实测验证。

### 九、时间戳始终可见 + 流式完成时间戳

**更新日志原文**：显示流式完成时间戳 (#5742)；时间戳始终可见；修复 fetch URL 中的双 /api 前缀 (#5802)

**与本插件关系**：低——但「时间戳始终可见」说明 v2.0 的气泡 DOM 结构可能含时间戳元素，本插件的 `childIndex` 算法（按 `<strong>`/`<li>` 序号）需确认时间戳不混入候选。

### 十、AgentScope middleware hook + semver 版本管理

**更新日志原文**：插件：AgentScope 中间件 hook：插件可注册 AgentScope 兼容的 middleware，带 semver 版本管理 (#5221)

**与本插件关系**：低——本插件是前端插件，不注册后端 middleware。但说明 v2.0 的插件系统更开放，未来本插件可考虑加后端组件暴露 history.db 查询。

### 十一、版本门控目录 + 插件版本保留

**更新日志原文**：版本门控目录：插件市场和 CDN 仅展示与当前 QwenPaw 版本兼容的插件 (#5661)；插件版本保留：更新后旧版本保留在磁盘上；加载失败状态下次启动时清理 (#5695)

**与本插件关系**：中——本插件的 `plugin.json` 需加 `qwenpaw_version`（§32.5i 已记计划）。「插件版本保留」说明 v2.0 会在磁盘保留旧版——本插件需确保 `dist/index.js` 的构建产物不依赖旧版 API。

### 十二、Tauri 桌面端 + DevTools 手势

**更新日志原文**：Tauri 迁移：桌面构建从 Electron 切换到 Tauri (#5734)；DevTools 手势：连击 8 次 Logo 打开 Chromium DevTools 调试 (#5805)

**与本插件关系**：低——本插件是前端插件，Tauri 的 native webview 应兼容 Chrome DevTools API。但需实测确认 Tauri webview 的 React fiber 遍历是否仍正常。

### 十三、移动端适配 + 响应式 CSS

**更新日志原文**：移动端适配：响应式 CSS + 抽屉式导航覆盖所有控制台页面 (#5444–#5470)

**与本插件关系**：中——本插件的 BarStrip 是左侧固定宽度，移动端可能被抽屉式导航覆盖。需实测验证 + 考虑加响应式 CSS 适配移动端。

### 十四、Token 用量弹窗 + 状态栏实时显示

**更新日志原文**：Token 用量弹窗：环形图展示每轮 token/context 用量 (#5493, #5544)

**与本插件关系**：低——但说明 v2.0 的前端有 token 用量数据，本插件未来可考虑加「按 token 用量排序条目」或「高亮 token 消耗大的工具调用」功能。

### 改进建议优先级

| 优先级 | 改进 | 改动量 | 收益 |
|:--|:--|:--|:--|
| **P0** | `plugin.json` 加 `qwenpaw_version`（§32.5i 已记） | 小 | 兼容性声明必需 |
| **P0** | 实测验证 9 项运行时差异（DOM class / React fiber / API 结构等） | 验证 | 确认 v2.0 兼容性 |
| **P1** | 从 BubbleList fiber 拿 `onEnsureMessageVisible` 闭包直调 | 中 | 远距离跳转从 ~3-5s 降到近瞬时 |
| **P1** | BarStrip 改用 `react-window` 虚拟化 | 中 | 长对话性能从 O(全量) 降到 O(可见) |
| **P2** | `fetchAndParse` 加 `AbortController` + 超时 | 小 | 防网络卡死无反馈 |
| **P2** | `toolCallParser` 加解析错误状态 tool call | 小 | 用户看到工具失败高亮 |
| **P2** | 加 prev/next 导航按钮（仿官方锚点） | 小 | 用户体验提升 |
| **P3** | 查 v2.0 是否有前端 API 访问 history.db | 调研 | 绕过 SDK 分页渲染限制 |
| **P3** | 移动端响应式 CSS 适配 | 中 | 移动端不被抽屉覆盖 |
| **P3** | 按 token 用量排序/高亮条目 | 中 | 未来功能增 |
